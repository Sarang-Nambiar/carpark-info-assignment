...
const db = new sqlite3.Database(":memory:", async (err) => {
  ...
  setupDatabase();
  data = parseCSV("./data/hdb-carpark-information-20220824010400.csv"); // replace this with the file parser of your choice
...
async function deleteData(row) {
  ...
  if (locResult) {
    locationID = locResult.id;
    await runQuery(`DELETE FROM CarPark_Location WHERE locationID = ?`, [
      locationID,
    ]);
  } else {
    // Current implementation would throw an error if the locationID isn't found
  ...
  if (policyResult) {
    policyID = policyResult.policyID;
    await runQuery(`DELETE FROM CarPark_Policy WHERE policyID = ?`, [policyID]);
  } else {
    throw new Error("Policy not found");
  }
  ...
  if (freeParkingResult) {
    freeparkingID = freeParkingResult.free_parkingID;
    await runQuery(`DELETE FROM CarPark_FreeParking WHERE free_parkingID = ?`, [
      freeparkingID,
    ]);
  } else {
    throw new Error("Free parking not found");
  ...
  if (typeResult) {
    typeID = typeResult.typeID;
    await runQuery("DELETE FROM CarPark_Type WHERE typeID = ?", [typeID]);
  } else {
    throw new Error("Type not found");
  }
